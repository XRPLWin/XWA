...
class Parser
{
  ...
  /**
   * Parses raw intersected transaction counts and generates paged scanplan.
   * @see tests/ScanplanParserTest tests for testable samples.
   * @return array [ <page> => [ <Txtypenamepart> => [ (int)total, (int)found, (string)e, (int)ledgerindex_first, (int)ledgerindex_last, (string)ledgerindex_last_id ], ... ], ... ]
   */
    ...
    foreach($paged_data as $page => $txtypes) {
      foreach($txtypes as  $txtype => $txtypeSubpages) {
        ...
        foreach($txtypeSubpages as $subpageCounts) {
          ...
          $_ledgerindex_last_ids[] = $subpageCounts['li_id']; //informational only, not used
        }
        ...
        $scanplan[$page][$txtype] = [
          ...
          'ledgerindex_last_id' => \max($_ledgerindex_last_ids) //informational only, not used
        ];
      }
    }
    return $scanplan;
   ...
   * eq - equal; lte - less than or equal
  ...
  /**
   * @return array [ <LedgerindexID> => [ sub-page => [ TxType => [ total,found,e,first,next,first2,next2 ] ] ] ... ]
   */
  ...
  private function runOneIndex(array $data) : array
  {
      ...
      foreach($this->data as $txType => $list) { //each txtype
        ...
        foreach($list as $li_subpage => $d) { //1234.001..9
          ...
          if($p[0] === $first || $p[1] === $last) {
            //dump('Page no'.$page.': '.$p[0].' == '.$first.' || '.$p[1].' == '.$last.' = '. (int)($p[0] == $first && $p[1] == $last));
            $valid = true;
            if($p[0] !== $first || $p[1] !== $last) {
              $e = 'lte';
            }
    ...
    foreach($collection as $page => $pagedata) {
      ...
      foreach($pagedata as $txtype => $counts)
      {
        ...
        # remove irrelevant first and last keys
  ...
  /**
   * @return array [  1827 => [ "1827.0001", "1827.0002", ... ], ...  ]
   */
  private function getAllLedgerIndexesIDs(): array
  {
    $keys = [];
