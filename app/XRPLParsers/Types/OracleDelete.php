...
final class OracleDelete extends XRPLParserBase
{
  private array $acceptedParsedTypes = ['UNKNOWN','SET','REGULARKEYSIGNER'];
  ...
  protected function parseTypeFields(): void
  {
    ...
    if(!in_array($parsedType, $this->acceptedParsedTypes))
      throw new \Exception('Unhandled parsedType ['.$parsedType.'] on OracleDelete with HASH ['.$this->data['hash'].'] and perspective ['.$this->reference_address.']');
    ...
    if(isset($this->data['Fee']) && $this->data['Fee']) {
      $this->persist = true;
    }
