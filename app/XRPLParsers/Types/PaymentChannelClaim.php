...
final class PaymentChannelClaim extends XRPLParserBase
{
  ...
  protected function parseTypeFields(): void
  {
    ...
    if(!in_array($parsedType, $this->acceptedParsedTypes))
      throw new \Exception('Unhandled parsedType ['.$parsedType.'] on PaymentChannelClaim with HASH ['.$this->data['hash'].']');
    ...
    if(isset($this->data['eventList']['primary'])) {
      ...
      if($this->data['eventList']['primary']['currency'] !== 'XRP') {
        throw new \Exception('Unhandled non XRP value on PaymentChannelClaim with HASH ['.$this->data['hash'].']');
      }
    }
